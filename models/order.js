const db = require('../config/config');

const Order = {};


Order.findByStatus = (status, result) => {

    const sql = `
     SELECT 
        CONVERT(O.id, char) as id, 
        CONVERT(O.id_client, char) AS id_client,
        CONVERT(O.id_address, char) AS id_address,
        CONVERT(O.id_delivery, char) AS id_delivery,
        O.timestamp,
        O.image,
        O.status,
        JSON_OBJECT(
            'id', CONVERT(A.id, char),
            'address', A.address,
            'neighborhood', A.neighborhood,
            'lat', A.lat,
            'lng', A.lng
        ) AS address,
        JSON_OBJECT(
            'id', CONVERT(U.id, char),
            'name', U.name,
            'lastname', U.lastname,
            'image', U.image,
            'phone', U.phone
        ) AS client,
        JSON_OBJECT(
            'id', CONVERT(D.id, char),
            'name', D.name,
            'lastname', D.lastname,
            'image', D.image,
            'phone', D.phone
        ) AS delivery,
        json_arrayagg(
            json_object(
                'id', CONVERT(P.id, char),
                'name', P.name,
                'description', P.description,
                'image1', P.image1,
                'image2', P.image2,
                'image3', P.image3,
                'price', P.price,
                'quantity', OHP.quantity
            )
        ) AS products
        
    FROM orders O
    INNER JOIN address A ON A.id = O.id_address
    INNER JOIN users U ON U.id = O.id_client
    LEFT JOIN users D ON D.id = O.id_delivery
    INNER JOIN order_has_products OHP ON OHP.id_order = O.id
    INNER JOIN products P ON OHP.id_product= P.id
    WHERE
        O.status = ?
    GROUP BY O.id
    `;

    db.query(
        sql,
        status,
        (err, data) => {
            if (err) {
                console.log('Error:', err);
                result(err, null);
            }
            else {
                result(null, data);
            }
        }
    )
}
Order.findByDeliveryAndStatus = (id_delivery, status, result) => {

    const sql = `
    SELECT 
        CONVERT(O.id, char) as id, 
        CONVERT(O.id_client, char) AS id_client,
        CONVERT(O.id_address, char) AS id_address,
        CONVERT(O.id_delivery, char) AS id_delivery,
            O.timestamp,
            O.image,
        O.status,
        JSON_OBJECT(
            'id', CONVERT(A.id, char),
            'address', A.address,
            'neighborhood', A.neighborhood,
            'lat', A.lat,
            'lng', A.lng
        ) AS address,
        JSON_OBJECT(
            'id', CONVERT(U.id, char),
            'name', U.name,
            'lastname', U.lastname,
            'image', U.image,
            'phone', U.phone
        ) AS client,
        JSON_OBJECT(
            'id', CONVERT(D.id, char),
            'name', D.name,
            'lastname', D.lastname,
            'image', D.image,
            'phone', D.phone
        ) AS delivery,
        json_arrayagg(
            json_object(
                'id', CONVERT(P.id, char),
                'name', P.name,
                'description', P.description,
                'image1', P.image1,
                'image2', P.image2,
                'image3', P.image3,
                'price', P.price,
                'quantity', OHP.quantity
            )
        ) AS products
        
    FROM orders O
    INNER JOIN address A ON A.id = O.id_address
    INNER JOIN users U ON U.id = O.id_client
    LEFT JOIN users D ON D.id = O.id_delivery
    INNER JOIN order_has_products OHP ON OHP.id_order = O.id
    INNER JOIN products P ON OHP.id_product= P.id
    WHERE
        O.id_delivery = ? AND O.status = ?
    GROUP BY O.id
    `;

    db.query(
        sql,
        [id_delivery, status],
        (err, data) => {
            if (err) {
                console.log('Error:', err);
                result(err, null);
            }
            else {
                result(null, data);
            }
        }
    )
}

Order.findByClientAndStatus = (id_client, status, result) => {

    const sql = `
   SELECT 
        CONVERT(O.id, char) as id, 
        CONVERT(O.id_client, char) AS id_client,
        CONVERT(O.id_address, char) AS id_address,
        CONVERT(O.id_delivery, char) AS id_delivery,
        O.timestamp,
        O.image,
        O.status,
        JSON_OBJECT(
            'id', CONVERT(A.id, char),
            'address', A.address,
            'neighborhood', A.neighborhood,
            'lat', A.lat,
            'lng', A.lng
        ) AS address,
        JSON_OBJECT(
            'id', CONVERT(U.id, char),
            'name', U.name,
            'lastname', U.lastname,
            'image', U.image,
            'phone', U.phone
        ) AS client,
        JSON_OBJECT(
            'id', CONVERT(D.id, char),
            'name', D.name,
            'lastname', D.lastname,
            'image', D.image,
            'phone', D.phone
        ) AS delivery,
        json_arrayagg(
            json_object(
                'id', CONVERT(P.id, char),
                'name', P.name,
                'description', P.description,
                'image1', P.image1,
                'image2', P.image2,
                'image3', P.image3,
                'price', P.price,
                'quantity', OHP.quantity
            )
        ) AS products
        
    FROM orders O
    INNER JOIN address A ON A.id = O.id_address
    INNER JOIN users U ON U.id = O.id_client
    LEFT JOIN users D ON D.id = O.id_delivery
    INNER JOIN order_has_products OHP ON OHP.id_order = O.id
    INNER JOIN products P ON OHP.id_product= P.id
    WHERE
        O.id_client = ? AND O.status = ?
    GROUP BY O.id
    `;

    db.query(
        sql,
        [id_client, status],
        (err, data) => {
            if (err) {
                console.log('Error:', err);
                result(err, null);
            }
            else {
                result(null, data);
            }
        }
    )
}

Order.create = (order, result) => {

    const sql = `
    INSERT INTO
        orders(
            id_client,
            id_address,
            status,
            timestamp,
            created_at,
            updated_at,
            image
        )
    VALUES(?, ?, ?, ?, ?, ?, ?)
    `;

    db.query(
        sql,
        [
            order.id_client,
            order.id_address,
            'REVISION', // 1. PAGADO 2. DESPACHADO 3. EN CAMINO 4. ENTREGADO
            Date.now(),
            new Date(),
            new Date(),
            order.image
        ],
        (err, res) => {
            if (err) {
                console.log('Error:', err);
                result(err, null);
            }
            else {
                console.log('Id de la nueva orden:', res.insertId);
                result(null, res.insertId);
            }
        }

    )
}



Order.updateToPay = (id_order, id_delivery, result) => {
    const sql = `
    UPDATE
    orders
    SET
        id_delivery = ?,
        status = ?,        
        updated_at= ?   
    WHERE
        id = ?
        `;

        db.query(
        sql,
        [
            id_delivery,
            'PAGADO',
            new Date(),
            id_order
        ],
        (err, res) => {
            if (err) {
                console.log('Error de cambiar el estado de revision a pagado: ', err);
                result(err, null);
            } else {
                result(null, id_order);
            }
        }
    )
}


Order.updateToDispatched = (id_order, id_delivery, result) => {
    const sql = `
    UPDATE
        orders
    SET
        id_delivery = ?,
        status = ?,
        updated_at = ?
    WHERE
        id = ?
    `;

    db.query(
        sql,
        [
            id_delivery,
            'DESPACHADO',
            new Date(),
            id_order
        ],
        (err, res) => {
            if (err) {
                console.log('Error:', err);
                result(err, null);
            }
            else {
                result(null, id_order);
            }
        }
    )
}

Order.updateToOnTheWay = (id_order, id_delivery, result) => {
    const sql = `
    UPDATE
        orders
    SET
        id_delivery = ?,
        status = ?,
        updated_at = ?
    WHERE
        id = ?
    `;

    db.query(
        sql,
        [
            id_delivery,
            'EN CAMINO',
            new Date(),
            id_order
        ],
        (err, res) => {
            if (err) {
                console.log('Error:', err);
                result(err, null);
            }
            else {
                result(null, id_order);
            }
        }
    )
}

Order.updateToDelivered = (id_order, id_delivery, result) => {
    const sql = `
    UPDATE
        orders
    SET
        id_delivery = ?,
        status = ?,
        updated_at = ?
    WHERE
        id = ?
    `;

    db.query(
        sql,
        [
            id_delivery,
            'ENTREGADO',
            new Date(),
            id_order
        ],
        (err, res) => {
            if (err) {
                console.log('Error:', err);
                result(err, null);
            }
            else {
                result(null, id_order);
            }
        }
    )
}


module.exports = Order;